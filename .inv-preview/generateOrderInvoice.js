...
const GREEN_DARK = [31, 82, 48]; // table head / brand
const GREEN_TXT = [27, 74, 44];
const GREEN_PALE = [214, 234, 214]; // grand-total highlight
const AMBER = [181, 133, 31]; // tagline / divider
const WHITE = [255, 255, 255];
const INK = [17, 24, 39];
const SLATE = [90, 100, 110];
const LIGHT = [120, 130, 140];
// ── Helpers ───────────────────────────────────────────────────────────────────
...
function generateOrderInvoice(data) {
    ...
    const addrLines = data.address
        ...
        : [];
    addrLines.forEach((line) => { doc.text(line, mg, by); by += 4.6; });
    ...
    const discount = (_a = data.discountAmount) !== null && _a !== void 0 ? _a : 0;
    const subtotal = (_b = data.subtotal) !== null && _b !== void 0 ? _b : (sumTaxable + sumIgst);
    ...
    if (discount > 0) {
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(9.5);
        doc.text((_c = data.discountLabel) !== null && _c !== void 0 ? _c : 'Prepaid Discount (10%)', totLabelX, y);
    ...
    doc.setFontSize(8.5);
    doc.setTextColor(...SLATE);
    doc.text(`Amount in words: ${numberToWords(data.totalAmount)}. Prices are inclusive of GST.`, mg, y + 2);
    // ═════════════════════════════════════════════════════════════════════════════════
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(...LIGHT);
    doc.text('Use products as directed. For best results, consult your physician. | This is a computer-generated invoice.', pageW / 2, footY + 14, { align: 'center' });
    doc.text(`${MFG_LIC} | Manufactured by ${MANUFACTURER}`, pageW / 2, footY + 19, { align: 'center' });
    // ── Save ───────────────────────────────────────────────────────────────────
